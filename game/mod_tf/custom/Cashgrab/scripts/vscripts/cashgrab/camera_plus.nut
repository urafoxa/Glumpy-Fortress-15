...
m_iPitchPoseParameter <- self.LookupPoseParameter( "aim_pitch" );
m_iYawPoseParameter <- self.LookupPoseParameter( "aim_yaw" );
m_iRightBound <- 45;
m_iLeftBound <- 315;
m_iBaseTurnRate <- 6;
m_flFieldOfView <- 0.7;
m_flTurnRate <- 0;
m_hEnemy <- GetListenServerHost();
m_vecCurAngles <- QAngle();
m_vecGoalAngles <- QAngle();
SENTRY_THINK_DELAY <- 0.05;

AddThinkToEnt(self,"Track")
