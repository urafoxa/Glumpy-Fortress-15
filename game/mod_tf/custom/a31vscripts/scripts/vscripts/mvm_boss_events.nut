...
const MVM_BOSS_DEATH_SOUND = "MVM.TankExplodes"
const IDLE_SOUND = "mvm/giant_pyro/giant_pyro_loop.wav"
PrecacheSound(IDLE_SOUND);
PrecacheScriptSound("MVM_Weapon_BaseballBat.HitFlesh");
PrecacheScriptSound(MVM_BOSS_DEATH_SOUND)

ClearGameEventCallbacks()

function OnPostSpawn()
{
	AddThinkToEnt(self, "Update_MVM_Bosses")
}

function Update_MVM_Bosses()
{
...
function OnGameEvent_npc_hurt(params)
{
	...
	if (nextbots.find(npc) != null && params.health - params.damageamount <= 0)
	{
		...
		npc.EmitSound(MVM_BOSS_DEATH_SOUND)
		local deathexp = SpawnEntityFromTable("info_particle_system", {
			effect_name = "mvm_tank_destroy",
			origin = npc.GetOrigin() + Vector(0, 0, 42),
			start_active = 1
		})
		EmitSoundEx(
		{
			sound_name = IDLE_SOUND,
			sound_level = 80,
			entity = npc,
			pitch = 85,
			flags = 4
...
{
	for (local NPC_Menaces; NPC_Menaces = Entities.FindByClassname(NPC_Menaces, "MVM_Boss_Menace");)
	EmitSoundEx(
		{
			sound_name = IDLE_SOUND,
			sound_level = 80,
