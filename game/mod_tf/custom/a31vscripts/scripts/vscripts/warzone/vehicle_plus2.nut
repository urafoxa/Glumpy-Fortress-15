...
local CarHandling = {
	...
	"models/props_vehicles_drive/morevehicles/truck_kenworth.mdl" : {TankVoice = false, HasWeapons = false, sfx_horn = "vehicles/drive/horn/horn_22_neutral.wav", delay_horn = 2.5},
	"models/props_vehicles_drive/krazyzark/spartan_jeep.mdl" : {TankVoice = false, HasWeapons = false, sfx_horn = "ambient/mvm_warehouse/car_horn_03.wav", delay_horn = 0.8},
...
local SeatTable = {
    ...
    "models/props_vehicles_drive/morevehicles/truck_kenworth.mdl" : { Seats = [Vector(0,0,0)] }
    "models/props_vehicles_drive/krazyzark/spartan_jeep.mdl" : { Seats = [Vector(0,0,0)] }
    "models/props_vehicles_drive/morevehicles/belair.mdl" : { SpecialSeats = ["S","SI","I"], Seats = [Vector(0,0,20), Vector(0,0,50), Vector(0,0,90)] }
  ...
  //"models/buggy.mdl" : { SpecialSeats = ["GSI","SI","G"], Seats = ["vehicle_driver_eyes", Vector(0,50,0), Vector(0,80,0)] }
    "models/airboat.mdl" : { SpecialSeats = [1,2,3], Seats = [Vector(0,0,0), "vehicle_driver_eyes", "vehicle_driver_eyes"] }
    "models/combine_apc.mdl" : { SpecialSeats = [1,2,3], Seats = [Vector(0,0,0), Vector(0,0,128), "vehicle_driver_eyes"] }
}
// SOUND TABLE
local SoundTable = {
    ...
    "models/props_vehicles_drive/morevehicles/truck_kenworth.mdl" : {SFX_IDLE = "vehicles/drive/truck_idle.wav" , SFX_ACCEL = "vehicles/drive/truck_accel.wav" , SFX_TURBO = "vehicles/v8/v8_turbo_on_loop1.wav", SFX_SLOW = "vehicles/drive/truck_decel.wav" }
    "models/buggy.mdl" : {SFX_IDLE = "vehicles/v8/v8_idle_loop1.wav" , SFX_ACCEL = "vehicles/v8/v8_firstgear_rev_loop1.wav" , SFX_TURBO = "vehicles/v8/v8_turbo_on_loop1.wav", SFX_SLOW = "vehicles/v8/v8_throttle_off_fast_loop1.wav" }
    "models/props_vehicles_drive/krazyzark/spartan_jeep.mdl" : {SFX_IDLE = "vehicles/drive/hl2/jnk_idle_loop1.wav" , SFX_ACCEL = "vehicles/drive/hl2/jnk_firstgear_rev_loop1.wav" , SFX_TURBO = "vehicles/drive/hl2/jnk_turbo_on_loop1.wav", SFX_SLOW = "vehicles/drive/hl2/jnk_throttle_off_fast_loop1.wav" }
}
...
local RespawnTable = {
	...
	"drv_car_belair_blu_*" : {Respawner = "respawner_counter_blu_1"},
}
...
::VehicleThink <- function()
{
	for (local player; player = Entities.FindByClassname(player, "player");)
	{
		...
		{	
			...
			TraceLineEx(trace);	
			if (trace.hit && (Entities.FindByClassname(null, "tf_logic_mann_vs_machine") ? NetProps.HasProp(trace.enthit, "m_bHasGun") : trace.enthit.GetClassname() == "prop_vehicle_driveable"))
			{
...
// This hack allows vehicle damage to show the train kill icon
...
function Precache()
{
	...
	fixup_origin <- Vector();
	fixup_angles <- QAngle();
	player_array <- array((vehicle.GetModelName() in SeatTable) ? SeatTable[vehicle.GetModelName()].Seats.len() : 0,[null, EXIT_DELAY]) // put it in the precache of the vehicle.
	...
	{	
		...
		NetProps.SetPropBool(CarWeaponMimic, "m_bForcePurgeFixedupStrings", true)
	...
	if (Entities.FindByClassname(null, "tf_logic_mann_vs_machine"))
		vehicle.KeyValueFromString("classname", "info_target")
}
...
function CarWeapon_Primary(vehicle)
{
	...
	local SpawnedProjectile = Entities.FindByClassnameNearest("tf_projectile_rocket", CarWeaponMimic.GetOrigin(), 20)
	NetProps.SetPropBool(SpawnedProjectile, "m_bForcePurgeFixedupStrings", true)
...
function ExplodeCar()
{
	foreach(key, value in RespawnTable)
	{
		local carname = key
		if(key.find("*") != null)
			{
			carname = key.slice(0, key.find("*"))
			}
		//printl(carname)
		if(vehicle.GetName().find(carname) != null)
		{
			if("RespawnTimer" in value)
			{
			printl("Timed Spawner")
			EntFire(value.Respawner, "ForceSpawn", "0", value.RespawnTimer, null)
			}
			else
			{
			printl("Counter Spawner")
			EntFire(value.Respawner, "Add", "1", 0, null)
			}
		}
	}
	IsDestroyed = true
	...
	NetProps.SetPropBool(DeathExplosion, "m_bForcePurgeFixedupStrings", true)
	...
	foreach (i, player in player_array){
		if (player[0])
		{
			Passenger_Exit(player[0])
			player_array[i] = [null, EXIT_DELAY]
		}
	}
	//Chain Reaction other cars
	if (Entities.FindByClassname(null, "tf_logic_mann_vs_machine"))
	{
		for (local DamageOther; DamageOther = Entities.FindByClassnameWithin(DamageOther, "info_target", vehicle.GetOrigin(), DamageTable[vehicle.GetModelName()].DamageRadius);)
		{
			if (NetProps.HasProp(DamageOther, "m_bHasGun"))
			{
				if (DamageOther.GetModelName() in DamageTable)
				{
...
function Enter_proxy(player, forceseat = -1)
{
	...
	local player_array_temp = []
	foreach (item in player_array)
	{
		player_array_temp.append(item[0])
	}
	if (player_array_temp.find(player) == null)
	{
		local seat_index = 0
		if (forceseat > -1)
		{
			seat_index = forceseat
			player_array[forceseat] = [player, EXIT_DELAY]
		}
		else
		{
			foreach (i, element in player_array)
			{
				if (!CBaseEntity.IsValid.call(element[0]) && element[0] != player && (driver && forceseat < 0))
				{
					player_array[i] = [player, EXIT_DELAY]
					seat_index = i
					break;
				}
			}
		...
		if (!CBaseEntity.IsValid.call(driver) && forceseat < 0)
		{
			Enter(player)
		}
		else if (vehicle.GetModelName() in SeatTable)
		{
...
function CanExit_passenger(player)
{
...
function Passenger_Exit(player, dead = false)
{	
	if (!dead)
		NetProps.SetPropBool(player, "pl.deadflag", false);
	...
	fixup_origin = player.GetOrigin() + Vector(0, 0, 8);
	fixup_angles = player.EyeAngles();				

	if (!CanExit_passenger(player))
	{
		// too bad
		//printl("Can't exit!");
	}
	...
	player.SetAbsOrigin(fixup_origin);
	player.SnapEyeAngles(fixup_angles);
	player.SetAbsVelocity(vehicle.GetPhysVelocity());		
	...
	local player_scope = player.GetScriptScope();
	player_scope.vehicle = null;
	player_scope.vehicle_scope = null;
}
	...
	if (driver)
	{
		...
		if (!CarHandling[vehicle.GetModelName()].TankVoice)
		{
			if (snd_turbo_cooldown < Time() && buttons & IN_RELOAD)
			{
				...
				EmitSoundEx({ sound_name = SoundTable[vehicle.GetModelName()].SFX_ACCEL, volume = 1, flags = 4, sound_level = 80, filter_type = 5, entity = vehicle });
				EmitSoundEx({ sound_name = SoundTable[vehicle.GetModelName()].SFX_TURBO, volume = 1, sound_level = 80, entity = vehicle, filter_type = 5});
				snd_engine_state = "T"
			}
			if (Turboing == true && snd_turbo_cooldown < Time() + 0.1)
			{
				...
				if (snd_engine_state == "T")
				{
					...
					{
						EmitSoundEx({ sound_name = SoundTable[vehicle.GetModelName()].SFX_SLOW, volume = 1, sound_level = 80, entity = vehicle, filter_type = 5});
						snd_engine_state = "NO"
					}
					else if (buttons & IN_FORWARD)
					{
						EmitSoundEx({ sound_name = SoundTable[vehicle.GetModelName()].SFX_ACCEL, volume = 1, sound_level = 80, entity = vehicle, filter_type = 5});
						snd_engine_state = "F"					
					}
				}
			}
...
::VehicleEvents =
{
	OnGameEvent_player_spawn = function(params)
	{
		...
		if (params.team & 2)
		{
			...
			if (temp_vehicle_scope)
			{
				if (temp_vehicle_scope.driver == player)
					temp_vehicle_scope.Exit(false, false);
				else
				{
					...
					{	
						if (item[0] == player)
						{
							temp_vehicle_scope.Passenger_Exit(player, true)
							temp_vehicle_scope.player_array[i] = [null, EXIT_DELAY];
							scope.vehicle = null;
							scope.vehicle_scope = null;
						}
					}
				}
				temp_vehicle_scope.update_passengers_state()
				player.ForceRespawn()
			}
		...
		if (scope && scope.vehicle_scope)
		{
			...
			else
			{
				...
				{	
					if (item[0] == player)
					{
						temp_vehicle_scope.Passenger_Exit(player)
						temp_vehicle_scope.player_array[i] = [null, EXIT_DELAY];
						scope.vehicle = null;
						scope.vehicle_scope = null;
					}
	...
	OnGameEvent_player_disconnect = function(params)
	{
		...
		{
			if (temp_vehicle_scope.driver == player)
				temp_vehicle_scope.Exit(true, false);
			else
			{
				foreach (i, item in temp_vehicle_scope.player_array)
				{	
    ...
    { 
		if (GetRoundState() != 3) return
		{
			...
			for (local vehicle; vehicle = Entities.FindByClassname(vehicle, "info_target");)
			{
				if (NetProps.HasProp(vehicle, "m_bHasGun"))
				{
					vehicle.Kill()
				}
			}
			EntFireByHandle(VehicleDmgOwner,"runscriptcode", "delete ::VehicleEvents", 0.0, null, null)
		}
    }
			...
			for (local vehicle; vehicle = Entities.FindByClassname(vehicle, "info_target");)
			{
				if (NetProps.HasProp(vehicle, "m_bHasGun"))
				{
					...
					foreach (i, player in vehicle.GetScriptScope().player_array)
					{
						...
						vehicle.GetScriptScope().player_array[i] = [null, EXIT_DELAY]
					}
				}
			}
		}
	...
	OnScriptHook_OnTakeDamage = function(params)
	{
		...
		if (Entities.FindByClassname(null, "tf_logic_mann_vs_machine") ? NetProps.HasProp(victim, "m_bHasGun") : victim.GetClassname() == "prop_vehicle_driveable")
		{
			// Minigun and sentry damage nerf
				...
				if (DamageTable[vehicle.GetModelName()].CanDamage)  
				{
					...
					if (MetalAmmo >= 50)
					{
						if (vehicle.GetHealth() < DamageTable[vehicle.GetModelName()].HP)
						{
							...
							{
							  //ClientPrint(null,3,"current fix amount: " + FixedHP.tostring())
							  //ClientPrint(null,3,"current HP amount: " + vehicle.GetHealth().tostring() + " / " + DamageTable[vehicle.GetModelName()].HP.tostring())
							  //ClientPrint(null,3,"proposed fix amount: " + (DamageTable[vehicle.GetModelName()].HP - vehicle.GetHealth()).tostring())
							  FixedHP = DamageTable[vehicle.GetModelName()].HP - vehicle.GetHealth()
							  vehicle.SetHealth(DamageTable[vehicle.GetModelName()].HP)
							}
							else
							...
							NetProps.SetPropIntArray(params.inflictor, "m_iAmmo", MetalAmmo - min(MetalAmmo,FixedHP), 3)
						}
						else
		...
		else if (inflictor && (Entities.FindByClassname(null, "tf_logic_mann_vs_machine") ? NetProps.HasProp(inflictor, "m_bHasGun") : inflictor.GetClassname() == "prop_vehicle_driveable"))
		{
		...
		if ((Entities.FindByClassname(null, "tf_logic_mann_vs_machine") ? NetProps.HasProp(vehicle, "m_bHasGun") : vehicle.GetClassname() == "prop_vehicle_driveable") && !vehiclescope.IsDestroyed)
		{
			if (vehicle.GetModelName() in DamageTable)
			{
				if (DamageTable[vehicle.GetModelName()].CanDamage)  
				{
					if (!vehicle.GetScriptScope().IsDestroyed)
					{
						if (vehiclescope.driver)
						{
							if (inflictor.GetTeam() != vehicle.GetScriptScope().DriversTeam || Convars.GetBool("mp_friendlyfire"))
							{
								...
								SendGlobalGameEvent("npc_hurt", {attacker_player  = GetPlayerUserID(params.attacker), health = vehicle.GetHealth(), damageamount = params.damage, entindex = vehicle.entindex()})
